...
SAVE_CQL = """INSERT INTO epsg_5070 (x, y, ubid, source, acquired, data,
...
FIND_CQL = """SELECT * FROM epsg_5070 WHERE
    x = ? AND y = ? AND ubid = ? AND acquired > ? AND acquired < ?"""

FIND = session.prepare(FIND_CQL)


def find(x, y, ubid, t1, t2):
    ...
    a usable array.
    """
    logger.debug("find <%s,%s> (%s) @ %s-%s." % (ubid, x, y, t1, t2))
    t1 = datetime.strptime(t1, "%Y-%m-%d")
    t2 = datetime.strptime(t2, "%Y-%m-%d")
    parameters = (x, y, ubid, t1, t2)
...
def find_area(x1, y1, x2, y2, ubid, t1, t2, grid=30 * 256):
    ...
    args = [(tx, ty, ubid, t1, t2) for ty in ys for tx in xs]
